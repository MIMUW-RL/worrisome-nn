...
class CartPoleSwingUpEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.state = None
        self.explicit = True
        self.no_terminate = False
        self.vel_penalty = 0.0

    def set_dt(self, dt):
        self.params.deltat = dt
    ...
    def set_no_terminate(self, no_terminate):
        self.no_terminate = no_terminate

    def set_vel_penalty(self, vel_penalty):
        self.vel_penalty = vel_penalty

    def set_precision(self, prec):
        self.precision = prec
        high = np.array([1.0], dtype=prec)
        self.action_space = spaces.Box(low=-high, high=high)
        high = np.array([np.finfo(prec).max] * 5, dtype=prec)
        self.observation_space = spaces.Box(low=-high, high=high)
    ...
    def step(self, action):
        ...
        next_obs = self._get_obs(next_state, self.precision)
        if self.vel_penalty:
            reward = self._reward_velpenal_fn(state, action, next_state, self.vel_penalty)
        else:
    ...
    def _transition_fn(self, state, action):
        ...
        ) / (4 * self.params.masstotal - 3 * self.params.pole.mass * cos_theta**2).astype(self.precision)
        thetadot_update = (
        ...
        ) / (4 * self.params.pole.length * self.params.masstotal - 3 * self.params.mpl * cos_theta**2).astype(
            self.precision
        )
    ...
    @staticmethod
    def _get_obs(state, precision=np.float32):
        x_pos, x_dot, theta, theta_dot = state
        return np.array([x_pos, x_dot, np.cos(theta), np.sin(theta), theta_dot], dtype=precision)

    def render(self, mode="human"):
        if self.viewer is None:
            self.viewer = CartPoleSwingUpViewer(self.params.cart, self.params.pole, world_width=5)
...
class CartPoleSwingUpV0(CartPoleSwingUpEnv):
    ...
    @staticmethod
    def _reward_velpenal_fn(state, action, next_state, vel_penalty=0.5):
        # return np.cos(next_state.theta, dtype=np.float32) - 0.5 * next_state.theta_dot**2
        return (
            np.cos(next_state.theta, dtype=np.float32)
            - vel_penalty * next_state.theta_dot**2
            - vel_penalty * next_state.x_dot**2
        )
